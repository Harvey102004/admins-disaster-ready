...
export default function EditDisasterForm() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: DisasterType) => {
      toast.loading("Updating Disaster Updates...");
      const res = await editDisasterUpdates({ id: editId, data });
      toast.dismiss();
      return res;
    },

    onSuccess: async () => {
      toast.success("Disaster updates updated successfully!");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      ...
      >
        <HiOutlineX
          onClick={() => router.push("/super-update-news/disaster-updates")}
